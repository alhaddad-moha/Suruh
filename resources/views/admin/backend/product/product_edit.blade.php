                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <h4 class="card-title">تعديل صنف </h4><br><br>


                            <form id="myForm" method="post" action="{{ route('product.update') }}">
                                @csrf
                                <div class="row mb-3">
                                    <label for="example-text-input" class="col-sm-2 col-form-label">اسم الصنف</label>
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="form-group col-sm-10">
                                        <select name="supplier_id" class="form-select"
                                            ...
                                            <option selected="">إختر شركة</option>
                                            @foreach($suppliers as $key => $item)
                                                <option
                                                    value="{{ $item->id}}" {{$product->supplier_id ? 'selected':''}} >{{ $item->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="form-group col-sm-10">
                                        <select name="category_id" class="form-select"
                                                aria-label="Default select example">
                                            <option selected="">إختر نوع</option>
                                            @foreach($categories as $key => $item)
                                                <option
                                                    value="{{ $item->id}}" {{$product->category_id ? 'selected':''}} >{{ $item->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="form-group col-sm-10">
                                        <select name="unit_id" class="form-select" aria-label="Default select example">
                                            <option selected="">إختر وحدة</option>
                                            @foreach($units as $key => $item)
                                                <option
                                                    value="{{ $item->id}}" {{$product->unit_id ? 'selected':''}} >{{ $item->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
